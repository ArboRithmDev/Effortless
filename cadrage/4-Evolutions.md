...
| EVO-013 | finding | Embed Obsidian : le miroir complet fuite un secret + 32 Mo de binaires | Planifié | — | — |
| EVO-014 | besoin | Améliorer la vue graphe : couleur par statut, titres frontmatter, relations | En cours | 008-Epic-Graphe | — |
| EVO-015 | finding | Le gate anti-dérive bloque les commits de fin de session (travail terminé, 0 tâche Doing) | Planifié | — | — |
...
- **EVO-013** — La config live cadrage/.obsidian contient un secret (clé privée RSA + apiKey dans plugins/obsidian-local-rest-api/data.json) et pèse 32 Mo (binaires plugins, sheet-plus main.js = 20 Mo). Embarquer ce miroir dans le template du scaffold (dépôt public) publierait la clé privée et gonflerait le repo. Décision révisée : embed CURÉ sans secret ni binaires — config JSON (app/appearance/core-plugins/graph/community-plugins) + dashboards 6-Suivi.base/6-Suivi-stats.md ; Obsidian propose l'install des plugins communautaires au 1er open. cadrage/.obsidian reste gitignored.
- **EVO-014** — Rendre le graphe Obsidian plus lisible : (1) couleur des nœuds par statut (colorGroups recherche propriété), (2) nom affiché depuis un champ titre du frontmatter (requiert d'ajouter titre aux docs story + config front-matter-title), (3) relations plus claires via extended-graph. Concerne le générateur cadrage_frontmatter (ajout titre) + config .obsidian.
- **EVO-015** — check_project_drift bloque dès qu'il y a des fichiers de travail modifiés ET 0 tâche Doing. En fin de session tout est Done (tâches/stories closes) → aucun Doing → le commit-checkpoint de travail COMPLET et TRACÉ est refusé, forçant --no-verify. Le gate ne distingue pas dérive (travail non tracké) et finalisation (travail tracké, tâches closes). Piste : autoriser le commit si les fichiers modifiés correspondent à des tâches/stories Done récentes, ou fournir un mode checkpoint explicite (effortless_checkpoint) qui pose une trace au lieu d'exiger un Doing.
