"""Double de test Jira en mémoire (STO-TRACKER-02 → 03).

`FakeJiraClient` simule l'exécuteur agent (Rovo) pour les tests hermétiques : il
enregistre les issues créées (parent, labels, type) et permet la recherche par
label. Le client REST + token de STO-TRACKER-02 a été retiré (DEC-02, projection
médiée agent) — plus aucun accès réseau côté serveur.
"""
